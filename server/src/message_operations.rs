...
pub async fn targets(
    ...
    let rows = sqlx::query("SELECT DISTINCT d.id, d.public_key FROM offline_messages m JOIN devices d ON d.id = m.recipient_device_id
        WHERE m.message_id = $1 AND m.from_user_id = $2 AND m.sender_device_id = $3 AND d.revoked = false")
...
pub async fn submit(
    ...
    let original = sqlx::query("SELECT from_user_id, sender_device_id, conversation_id, MIN(timestamp) AS timestamp FROM offline_messages
        WHERE message_id = $1 GROUP BY from_user_id, sender_device_id, conversation_id")
        .bind(&h.target_id).fetch_all(&mut *tx).await.map_err(database)?;
    if original.len() != 1 {
        return Err(reject("原消息不存在或编号冲突"));
    }
    let original = &original[0];
    if original.get::<String, _>("from_user_id") != user
    ...
    let now: i64 =
        sqlx::query_scalar("SELECT (EXTRACT(EPOCH FROM clock_timestamp()) * 1000)::BIGINT")
            ...
            .await
            .map_err(database)?;
    if now - original.get::<i64, _>("timestamp") > OPERATION_WINDOW_MS {
    ...
    let targets = sqlx::query("SELECT DISTINCT d.id FROM offline_messages m JOIN devices d ON d.id = m.recipient_device_id WHERE m.message_id = $1 AND d.revoked = false")
        .bind(&h.target_id).fetch_all(&mut *tx).await.map_err(database)?;
