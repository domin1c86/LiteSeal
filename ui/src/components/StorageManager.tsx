...
export default function StorageManager({ onClose }: StorageManagerProps) {
  const [cache, setCache] = useState<{ cache_bytes: number; database_allocated: number; database_reusable: number; limit: number } | null>(null);
      ...
      <div style={styles.modal}>
        ...
        {stats ? (
          <div style={styles.body}>
            {cache && <section><p>附件密文缓存：{formatBytes(cache.cache_bytes)} / {formatBytes(cache.limit)}。数据库已分配：{formatBytes(cache.database_allocated)}；内部可复用：{formatBytes(cache.database_reusable)}。</p>
              ...
              <button disabled={clearing} onClick={async () => {
                setClearing(true);
                try { const before = cache.cache_bytes; await window.desktop.clear_attachment_cache({ peerId: peerId || undefined }); const after = await window.desktop.attachment_cache_stats({}); setCache(after); setResult(`已移除 ${formatBytes(before - after.cache_bytes)} 密文缓存。SQLite 空间可复用，文件未必缩小；没有删除正文、链或待发任务。`); }
