...
完整入口、环境配置、邀请码、功能清单及手动测试用例见 [Windows 开发与测试指南](docs/WINDOWS_TESTING_GUIDE.md)。桥接协议及迁移说明见 [Electron 架构说明](docs/ELECTRON_ARCHITECTURE.md)。后续缺项、优先级和完成标准见 [开发清单](docs/DEVELOPMENT_BACKLOG.md)。
...
首次注册需要服务端配置的有效邀请码；密码至少 8 字符，确认密码需一致，各行有校验提示灯。现有功能还包括会话恢复与自动重连、联系人、指纹信任、加密单聊、草稿和原消息重试、历史分页、存储统计和清理。
