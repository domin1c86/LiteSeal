...
### P1 从本机删除消息（2026-09-13）

点击消息下方“从本机删除”，阅读说明并确认。消息从当前聊天、分页历史、会话摘要和未读数中移除，重启后仍隐藏。它不影响对方、不取消投递，也不会移除已有引用或转发快照。加密记录保留用于消息链校验，因此不等同于彻底擦除或释放存储空间。

正在发送或锁定为当前重试草稿的消息暂不能从本机删除。删除保存失败时保留原消息并显示错误，可再次操作。当前仍未提供编辑与远端撤回。桌面 API 总计 36 个；本轮仅通过编译/构建检查，未运行测试。


### D-03 完整消息操作（2026-09-13）

已完成剩余编辑和远端撤回入口。自己在当前设备发送且已到达服务端的消息，在首次服务端接收后的 48 小时内可编辑或撤回；服务端最终校验期限与权限。编辑后显示版本标记；撤回后显示占位，不再提供正文回复/复制/转发。变更保存后自动同步，离线设备重连补收；未确认操作会重试原编号，明确拒绝时显示原因。

变更不会修改原始密文链。此前“未提供编辑/撤回”的段落是历史开发记录，以本节为准。本机删除仍是逻辑删除，撤回也无法擦除对方已经复制或转发的内容。客户端与服务端必须统一升级；业务 API 现为 39 个。

规则和后续验收场景详见 [MESSAGE_OPERATIONS.md](MESSAGE_OPERATIONS.md)。本轮仅执行 Rust 编译、Electron 和前端构建，未运行测试，不宣称 Windows 或服务端联调验收通过。
